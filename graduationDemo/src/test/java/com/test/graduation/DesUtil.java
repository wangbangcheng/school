...
public class DesUtil {
	...
	//左移位数
	private static final int[] L_move={1,1,2,2,2,2,2,2,1,2,2,2,2,2,2,1};
    ...
    public DesUtil(String key,String content){
    ...
    public static void main(String[] args){
        ...
        DesUtil desUtil=new DesUtil("密码学专栏",origin);
        byte[] c=desUtil.deal(origin.getBytes(),1);
        System.out.println("密文：\n"+new String(c));
        byte[] p=desUtil.deal(c,0);
    ...
    public void generateKeys(String key){
        ...
        for(int i=0;i<PC_1.length;i++){
            k_new_bit[i]=k_bit[PC_1[i]-1];/////这个减1注意点
        }
        /**************************/
        int[] c0=new int[28];
        int[] d0=new int[28];
        ...
        for(int i=0;i<16;i++){
            ...
            for (int j=0;j<PC_2.length;j++){//PC2压缩置换
